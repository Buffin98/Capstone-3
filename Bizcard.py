...
icon = Image.open("OCR_icon.png")
st.set_page_config(page_title= "BizCardX: Extracting Business Card Data with OCR | By Buffin",
                   page_icon= icon,
                   layout= "wide",
...
st.markdown(f""" <style>.stApp {{
                    ...
                    height: 100%;
                    bottom: -1px;
                    background : url("https://www.shutterstock.com/image-illustration/da…lue-abstract-background-tech-260nw-1150857110.jpg");
...
if selected == "Home":
    ...
    with col1:
        ...
        st.write(" ")
        st.write(" ")
        st.markdown("## :rainbow[**Image Source:**]⬇️")
        st.write("https://stock.adobe.com/in/")
    with col2:
        st.image("Bizhome.jpg",use_column_width=True)
        st.image("Bizhome1.jpg",use_column_width=True)
...
# DISPLAYING THE DETAILS OF UPLOADED CARD
...
if selected == "Modify":
    ...
    try:
        with column1:
            ...
            cursor.execute("select company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code from card_details WHERE card_holder=%s",
                            (selected_card,))
            ...
            if st.button("Commit changes to DB"):
                ...
                cursor.execute("""UPDATE card_details SET company_name=%s,card_holder=%s,designation=%s,mobile_number=%s,email=%s,website=%s,area=%s,city=%s,state=%s,pin_code=%s
